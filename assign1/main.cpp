...
void parse_csv(std::string filename, std::vector<Course> &courses) {
  ...
  while (std::getline(ifs, record)) {
    if (record == "Title,Number of Units,Quarter") {
      continue;
    }
    std::vector<std::string>vec = split(record, ',');
    Course course;
    course.title = vec[0];
...
void write_courses_offered(std::vector<Course> &all_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::ofstream ofs(COURSES_OFFERED_PATH, std::ios::trunc);
  if (!ofs.good()) {
    std::cout << "open file fail!" << std::endl;
  }
...
void write_courses_not_offered(const std::vector<Course> unlisted_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::ofstream ofs(COURSES_NOT_OFFERED_PATH, std::ios::trunc);
  if (!ofs.good()) {
    std::cout << "open file fail!" << std::endl;
  }
  ofs << "Title,Number of Units,Quarter" << std::endl;
